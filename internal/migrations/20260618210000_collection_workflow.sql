...
CREATE INDEX IF NOT EXISTS idx_collections_workflow_state_id ON collections(workflow_state_id);

INSERT INTO workflows (key, name, description, entity_type) VALUES
    ('collection', 'Collection Lifecycle', 'Collection visibility and merchandising lifecycle', 'collection')
...
UPDATE collections c SET workflow_state_id = s.id
FROM workflow_states s
JOIN workflows w ON w.id = s.workflow_id AND w.key = 'collection'
WHERE c.workflow_state_id IS NULL
  AND s.code = CASE c.status
...
-- +goose Down
-- +goose StatementBegin
UPDATE collections SET workflow_state_id = NULL;
DELETE FROM workflow_transition_logs WHERE workflow_id IN (SELECT id FROM workflows WHERE key = 'collection');
DELETE FROM workflow_transitions     WHERE workflow_id IN (SELECT id FROM workflows WHERE key = 'collection');
DELETE FROM workflow_states          WHERE workflow_id IN (SELECT id FROM workflows WHERE key = 'collection');
DELETE FROM workflows WHERE key = 'collection';
ALTER TABLE collections DROP COLUMN IF EXISTS workflow_state_id;
-- +goose StatementEnd
