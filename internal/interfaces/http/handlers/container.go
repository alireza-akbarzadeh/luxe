...
func NewContainer(db *gorm.DB, apps *bootstrap.Applications, runtime *bootstrap.Runtime, cfg *config.Config) *Container {
	return &Container{
		...
		Ai:        NewAiHandler(apps.AI, apps.Search.Queries, apps.Compare.Queries, apps.Review.Queries, apps.Return.Queries, pdpPriceHistoryAdapter{svc: apps.Pdp}, shipmentDeliveryStatsAdapter{repo: postgres.NewShipmentRepository(db)}, apps.UserLike.Queries),
