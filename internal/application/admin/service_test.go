...
func setupTestDB(t *testing.T) (*gorm.DB, sqlmock.Sqlmock) {
	...
	gormDB, err := gorm.Open(pgdriver.New(pgdriver.Config{Conn: sqlDB}), &gorm.Config{})
	require.NoError(t, err)
	return gormDB, mock
}

func roleQueriesFromDB(db *gorm.DB) *approle.Queries {
	return approle.NewQueries(postgres.NewRoleRepository(db))
}

func TestAdminService_GetStats(t *testing.T) {
	db, mock := setupTestDB(t)
	svc := NewService(db, nil, roleQueriesFromDB(db))
